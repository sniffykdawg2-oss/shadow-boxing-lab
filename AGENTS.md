...
- Move frequency sliders are weights, not guaranteed exact percentages. Keep new cue keys coordinated across `MOVES`, `gameSettings.moveFrequencies`, the `[data-move-frequency]` controls, and the smoke-test offense/defense whitelists. Pivots are defensive end-of-combo calls and intentionally stay big-text even in realistic glove mode.
- `attackDefenseBias` nudges defensive cue chance while leaving the Offense/Defense checkboxes as hard gates. `rollBack` is a center head-movement cue and should stay grouped with rolls/head movement in stats, weights, tests, and glove demo animation.
